...
router.get('/load', requireAuth_1.requireAuth, async (req, res) => {
...
router.post('/save', requireAuth_1.requireAuth, async (req, res) => {
    const client = await pool_1.pool.connect();
    try {
...
router.get('/load_shared/:shareId', requireAuth_1.requireAuth, async (req, res) => {
    const client = await pool_1.pool.connect();
    try {
...
// POST /share/create
router.post('/share/create', requireAuth_1.requireAuth, async (req, res) => {
    const client = await pool_1.pool.connect();
    try {
        ...
        const updated = await client.query('UPDATE graphs SET share_id=$1 WHERE user_id=$2 RETURNING id', [shareId, req.session.userId]);
...
// POST /share/invite
router.post('/share/invite', requireAuth_1.requireAuth, async (req, res) => {
    const { email, share_id } = req.body;
    const client = await pool_1.pool.connect();
    try {
        const userRes = await client.query('SELECT id FROM users WHERE email=$1', [email.trim().toLowerCase()]);
        if (!userRes.rows[0]) {
            res.json({ error: 'User not found. Ask them to sign up first!' });
            return;
        }
        ...
        await client.query('INSERT INTO graph_collaborators (graph_id, user_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [graphRes.rows[0].id, userRes.rows[0].id]);
        res.json({ ok: true });
    }
    catch (_) {
...
// GET /api/dashboard
router.get('/api/dashboard', requireAuth_1.requireAuth, async (req, res) => {
    const client = await pool_1.pool.connect();
    try {
...
router.get('/api/collaborators/:shareId', requireAuth_1.requireAuth, async (req, res) => {
    const client = await pool_1.pool.connect();
    try {
        const result = await client.query(`
